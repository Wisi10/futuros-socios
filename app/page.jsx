...
// ─── Partner mapping ────────────────────────────────────────
const PARTNER_MAP = {
  'soukiwisam@gmail.com':'WISI',
  'ramzi@futuros.app':'RAMZI',
  'fvfc@futuros.app':'FVFC',
  'donaldo@futuros.app':'DONALDO',
  'gian@futuros.app':'GIAN',
  'yuyo@futuros.app':'YUYO',
  'david@futuros.app':'DAVID',
};
...
// Get partner's amount from a dividend row
...
class ErrorBoundary extends React.Component {
  ...
  render(){
    if(this.state.error) return <div style={{...S.card,borderColor:T.rd,margin:16}}>
      <div style={{...S.lbl,color:T.rd}}>ERROR</div>
      <pre style={{fontSize:11,color:T.rd,fontFamily:T.mo,whiteSpace:'pre-wrap',margin:'8px 0'}}>{this.state.error?.message}</pre>
    </div>;
...
function LoginScreen({onLogin}) {
  ...
  async function go(e) {
    ...
    // If no @, append @futuros.app
    const email=user.includes('@')?user:`${user.toLowerCase()}@futuros.app`;
...
// ═══════════════════════════════════════════════════════════════
// TAB 1 — EL COMPLEJO (same for all partners)
// ═══════════════════════════════════════════════════════════════
function TabComplejo({data}) {
  ...
  const typeBreakdown=useMemo(()=>{
    const map={};yb.forEach(b=>{const t=b.type||'otro';map[t]=(map[t]||0)+(b.price_eur||0)});
  ...
  const pyTypeMap=useMemo(()=>{
    const map={};(historicalSales||[]).filter(s=>s.sale_date?.startsWith(String(cy-1))).forEach(s=>{const t=s.court_type||'otro';map[t]=(map[t]||0)+(s.total_ref||0)});return map;
  },[historicalSales,cy]);
  ...
  const td=today,dow=now.getDay()||7;
  ...
  return <div className="fade-in">
    <div style={{...S.card,display:'flex',flexWrap:'wrap',justifyContent:'center'}}>
      ...
      <Vdiv/><KPI label="INGRESOS YTD" value={fmt(tr)} comp={yoyG!=null?`${cmp(tr,yoyG)} vs ${cy-1}`:`${cm} meses`}/>
      <Vdiv/><KPI label="TASA BCV" value={exchangeRate?`Bs ${Number(exchangeRate.eurRate).toLocaleString('es-VE',{maximumFractionDigits:0})}`:'—'} comp="Bs por REF" color={T.ch}/>
      <Vdiv/><KPI label={`OCUPACION ${MO[cm-1].toUpperCase()}`} value={`${tmH}h`} comp={`${cmp(tmH,occG)} vs ${MO[cm>1?cm-2:11].toLowerCase()}`} color={T.ch}/>
    </div>
    <div style={{...S.card,marginTop:12,display:'flex',flexWrap:'wrap',justifyContent:'center'}}>
      ...
      <Vdiv/><KPI label="CUMPLEANOS YTD" value={tbd.toString()} comp={`${avgBd.toFixed(1)}/mes`} color={T.ch}/>
      <Vdiv/><KPI label={`OCUPACION ${MO[cm-1].toUpperCase()}`} value={`${tmH}h`} comp={`${cmp(tmH,occG)} vs mes ant.`} color={T.ch}/>
    </div>
    ...
    <div style={S.card}>
      ...
      <div style={{height:180,marginTop:16}}>
        <Bar data={{labels:['2024','2025',`2026 YTD`],datasets:[{data:[rev24,rev25,rev26],backgroundColor:[T.dv,T.gold,T.gd],borderRadius:8,barThickness:40}]}} options={CO}/>
      </div>
    ...
    <div style={S.card}>
      ...
      <div style={{height:160,marginTop:16}}>
        <Bar data={{labels:MO,datasets:[
          {type:'line',label:'Promedio 2025',data:Array(12).fill(10),borderColor:'#B71C1C55',borderDash:[4,4],borderWidth:1.5,pointRadius:0,fill:false,order:0},
...
// ═══════════════════════════════════════════════════════════════
// TAB 2 — MI PARTICIPACION (partner-specific)
// ═══════════════════════════════════════════════════════════════
function TabParticipacion({data,partnerKey}) {
  ...
  const myRoi=roi?.[pk]||{};
  const myTotales=totales?.[pk]||{};
  const inv=myRoi.montoInvertido||0;
  const pct=myRoi.participacion||0;

  // Sum partner's dividends across all years
  ...
  const mtpb=myRoi.mesesRestantes||(avgM>0?Math.ceil(rem/avgM):null);
  ...
  const yTot=[
    myTotales.total2024||(dividends[2024]||[]).reduce((s,d)=>s+pAmt(d,pk),0),
    myTotales.total2025||(dividends[2025]||[]).reduce((s,d)=>s+pAmt(d,pk),0),
    myTotales.total2026||(dividends[2026]||[]).reduce((s,d)=>s+pAmt(d,pk),0),
  ];

  const mBreak=useMemo(()=>{
    const map={};(dividends[cy]||[]).forEach(d=>{const m=gm(d.fecha);const a=pAmt(d,pk);if(m&&a>0){if(!map[m])map[m]={month:m,amt:0,cnt:0};map[m].amt+=a;map[m].cnt++}});
  ...
  return <div className="fade-in">
    ...
    <div style={S.card}>
      ...
      <div style={{height:180,marginTop:16}}>
        <Bar data={{labels:['2024','2025','2026 YTD'],datasets:[{data:yTot,backgroundColor:[T.dv,T.gold,T.gd],borderRadius:8,barThickness:40}]}} options={CO}/>
      </div>
      <div style={{display:'flex',justifyContent:'space-between',marginTop:16}}>
    ...
    <div style={S.card}>
      ...
      <p style={{fontSize:12,lineHeight:1.7,color:T.ch,fontFamily:T.sa,margin:'12px 0 0'}}>
        En {monthsWithDivs} meses de {cy}, {pk} ha recibido <b style={{fontFamily:T.mo}}>{fmt(ytdDiv)}</b> en dividendos,
        ...
        {inv>0&&<> El ROI acumulado es <b>{fmtPct(roiPct)}</b> sobre una inversion de <b style={{fontFamily:T.mo}}>{fmt(inv)}</b>.</>}
...
// ═══════════════════════════════════════════════════════════════
// TAB 3 — PROYECCIONES (partner-specific)
// ═══════════════════════════════════════════════════════════════
function TabProyecciones({data,partnerKey}) {
  ...
  const rem=inv>0?(inv-tDiv):0;

  const sc=[
    {name:'Conservador',monthly:Math.round(curM*0.7)||800,color:T.dv,desc:'Ritmo reducido'},
    {name:'Base',monthly:Math.round(curM)||1000,color:T.gold,desc:'Ritmo actual'},
    {name:'Optimista',monthly:Math.round(curM*1.4)||1500,color:T.gd,desc:'Con mejoras operativas'},
  ];

  const proj=sc.map(s=>({...s,
    roi1y:inv>0?((tDiv+s.monthly*12)/inv*100):0,
    roi2y:inv>0?((tDiv+s.monthly*24)/inv*100):0,
    roi3y:inv>0?((tDiv+s.monthly*36)/inv*100):0,
    pbM:rem>0&&s.monthly>0?Math.max(0,Math.ceil(rem/s.monthly)):0,
  }));
  ...
  const myVal=complexVal*pct/100;
  ...
  return <div className="fade-in">
      ...
      <div key={i} style={{...S.card,borderLeft:`4px solid ${s.color}`,marginBottom:12}}>
        ...
        <div style={{display:'flex',marginTop:16,borderTop:`1px solid ${T.bg2}`,paddingTop:12}}>
          {[{l:'PAYBACK',v:s.pbM>0?`${s.pbM}m`:'Listo'},{l:'ROI 1A',v:fmtPct(s.roi1y)},{l:'ROI 2A',v:fmtPct(s.roi2y)},{l:'ROI 3A',v:fmtPct(s.roi3y)}].map((it,j)=>
            <div key={j} style={{flex:1,textAlign:'center'}}><div style={{fontSize:8,textTransform:'uppercase',letterSpacing:2,color:T.mu,fontFamily:T.sa}}>{it.l}</div><div style={{fontSize:13,fontWeight:700,color:T.ch,fontFamily:T.se,marginTop:4}}>{it.v}</div></div>
          )}
        </div>
      </div>
    )}
    ...
    <div style={S.card}>
      ...
      <div style={{marginTop:12}}>
        {[{y:'2 anos',v:r2y},{y:'3 anos',v:r3y},{y:'4 anos',v:r4y}].map((r,i)=>
          <div key={i} style={{...S.row,borderBottom:i<2?`1px solid ${T.bg2}`:'none'}}><span style={{fontSize:12,fontFamily:T.sa,color:T.ch}}>Payback en {r.y}</span><span style={S.mo}>{fmt(r.v)}/mes</span></div>
        )}
      </div>
    </div>
    ...
    <div style={S.card}>
      ...
      <div style={{display:'flex',gap:12,marginTop:16}}>
        {[{y:`Dic ${cy}`,v:roiEoy},{y:`Dic ${cy+1}`,v:roiEoy1},{y:`Dic ${cy+2}`,v:roiEoy2}].map((r,i)=>
          <div key={i} style={{flex:1,background:T.bg2,borderRadius:8,padding:16,textAlign:'center'}}><div style={{fontSize:9,textTransform:'uppercase',letterSpacing:2,color:T.mu,fontFamily:T.sa,marginBottom:6}}>{r.y}</div><div style={{fontSize:20,fontWeight:400,fontFamily:T.se,color:T.gold,letterSpacing:-1}}>{fmtPct(r.v)}</div></div>
        )}
      </div>
    </div>
    ...
    <div style={S.card}>
      ...
      <div style={{marginTop:12}}>
        {[{l:'Inversion original',v:fmt(inv)},{l:'Dividendos recibidos',v:`+${fmt(tDiv)}`,c:T.gr},{l:`Valor estimado complejo (10x rev.)`,v:fmt(complexVal)},{l:`Tu ${fmtPct(pct)}`,v:fmt(myVal),bold:true},{l:'Ganancia no realizada',v:unrealized>=0?`+${fmt(unrealized)}`:`-${fmt(Math.abs(unrealized))}`,c:unrealized>=0?T.gr:T.rd,bold:true}].map((r,i)=>
          <div key={i} style={{...S.row,borderBottom:i<4?`1px solid ${T.bg2}`:'none'}}><span style={{fontSize:12,fontFamily:T.sa,color:T.ch}}>{r.l}</span><span style={{...S.mo,...(r.bold?{fontSize:14}:{}),color:r.c||T.ch}}>{r.v}</span></div>
        )}
      </div>
    </div>
    </>}
      ...
      <div style={{maxWidth:520,margin:'0 auto',padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:9,color:T.mu,letterSpacing:2,textTransform:'uppercase',fontFamily:T.sa}}>{partnerKey} — {fmtPct(partnerPct)}</div>
          </div>
        </div>
